...
main(int argc, char **argv)
{
	...
	{
		strcpy(ticker, "FEC ");
		fprintf(stderr,"Generating FEC Address\n");
	}
	GRSFlag = 0;
...
/*BEGIN ALTCOIN GENERATOR*/
		...
		case 'C':
			...
			if (strcmp(optarg, "FEC")== 0) {
				fprintf(stderr,
						"Generating/Decrypting FEC [Ferritecoin] Address\n");
				addrtype_opt = 36;
				privtype_opt = 36+127;
				break;
			}
			else
			if (strcmp(optarg, "tFEC")== 0) {
				fprintf(stderr,
						"Generating/Decrypting tFEC [Ferritecoin] Testnet Address\n");
				addrtype_opt = 111;
				privtype_opt = 111+128;
				break;
			}
			else
